    ...
    <div class="min-h-screen bg-gray-950 flex items-center justify-center px-4">
      <div class="w-full max-w-md">
        <div class="text-center mb-8">
          ...
          <h1 class="text-2xl font-bold text-white">Sign in to <span class="text-blue-400">EduLibrary</span></h1>
        </div>
        <div class="bg-gray-900 border border-gray-800 rounded-2xl p-8">
          ...
          <p class="text-gray-500 text-sm text-center mt-6">
            No account? <a routerLink="/register" class="text-blue-400 hover:underline">Register here</a>
          </p>
        </div>
      </div>
    </div>
...
export class LoginComponent {
  ...
  onSubmit() {
    ...
    this.auth.login(this.form.value).subscribe({
      next: (res: any) => { this.loading = false; this.router.navigate([res.user.role === 'admin' ? '/admin' : '/']); },
      error: (err: any) => { this.loading = false; this.error = err.error?.message || 'Login failed.'; }
    });
  }
}